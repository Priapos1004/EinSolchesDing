...
- **`canvas-confetti` emoji shapes**: Use `shapeFromText({ text: '...', scalar: N })` and pass `scalar` to both the shape constructor AND the `confetti()` call. Missing the scalar on either side produces incorrectly sized particles.
